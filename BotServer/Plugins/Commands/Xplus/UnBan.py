...
matcher = on_command('unban', force_whitespace=True, block=True, priority=5, rule=Rules.command_rule)
...
async def handle_group(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if player := args.extract_plain_text().strip():
        message = await unban_handler(event, player)
        await matcher.finish(message, at_sender=True)
    await matcher.finish('请输入要绑定的玩家名称！')


# 处理绑定添加逻辑的异步函数
async def unban_handler(event: GroupMessageEvent, player: str):
    # 上锁以确保线程安全
    async with async_lock:
        ...
        await server_manager.execute(F'{config.unban_command} {player}')
